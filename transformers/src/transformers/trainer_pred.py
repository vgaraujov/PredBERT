...
class TrainerPred:
    ...
    def __init__(
        ...
        logger.info("Training run name: {}".format(self.args.run_name))
        assert (
    ...
    def train(self, model_path: Optional[str] = None, trial: Union["optuna.Trial", Dict[str, Any]] = None):
        ...
        tr_loss_mlm = torch.tensor(0.0).to(self.args.device)
        tr_loss_clm = torch.tensor(0.0).to(self.args.device)
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                if (
                    ...
                    with model.no_sync():
                        ...
                        tr_loss += losses[0]
                        tr_loss_mlm += losses[1]
                        tr_loss_clm += losses[2]
                else:
                    losses = self.training_step(model, inputs)
                    tr_loss += losses[0]
                    tr_loss_mlm += losses[1]
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0 or (
                    ...
                    self._maybe_log_save_evaluate(tr_loss, tr_loss_mlm, tr_loss_clm, model, trial, epoch)
    ...
    def _maybe_log_save_evaluate(self, tr_loss, tr_loss_mlm, tr_loss_clm, model, trial, epoch):
        if self.control.should_log:
            logs: Dict[str, float] = {}
            tr_loss_scalar = tr_loss.item()
            tr_loss_mlm_scalar = tr_loss_mlm.item()
            tr_loss_clm_scalar = tr_loss_clm.item()
            # reset tr_loss to zero
            tr_loss -= tr_loss
            tr_loss_mlm -= tr_loss_mlm
            tr_loss_clm -= tr_loss_clm

            logs["loss"] = tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged)
            logs["loss_mlm"] = tr_loss_mlm_scalar / (self.state.global_step - self._globalstep_last_logged)
            logs["loss_clm"] = tr_loss_clm_scalar / (self.state.global_step - self._globalstep_last_logged)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        if self.args.fp16 and _use_native_amp:
            with autocast():
                output = self.compute_loss(model, inputs)
                loss = output['loss']
                loss_clm = output['loss_clm']
                loss_mlm = output['loss_mlm']
        else:
            output = self.compute_loss(model, inputs)
            loss = output['loss']
            loss_clm = output['loss_clm']
        ...
        if self.args.n_gpu > 1:
            ...
            loss_clm = loss_clm.mean()  # mean() to average on multi-gpu parallel training

        if self.args.gradient_accumulation_steps > 1:
            loss = loss / self.args.gradient_accumulation_steps
            loss_mlm = loss_mlm / self.args.gradient_accumulation_steps
            loss_clm = loss_clm / self.args.gradient_accumulation_steps
        ...
        return loss.detach(), loss_mlm.detach(), loss_clm.detach()
    ...
    def compute_loss(self, model, inputs):
        ...
        return outputs if isinstance(outputs, dict) else outputs[0]
    ...
    def prediction_loop(
        ...
        losses_mlm_host: torch.Tensor = None
        losses_clm_host: torch.Tensor = None
        ...
        eval_losses_mlm_gatherer = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=batch_size)
        eval_losses_clm_gatherer = DistributedTensorGatherer(world_size, num_examples, make_multiple_of=batch_size)
        ...
        for step, inputs in enumerate(dataloader):
            loss, loss_mlm, loss_clm, logits, labels = self.prediction_step(model, inputs, prediction_loss_only, ignore_keys=ignore_keys)
            if loss is not None:
                losses = loss.repeat(batch_size)
                losses_mlm = loss_mlm.repeat(batch_size)
                losses_clm = loss_clm.repeat(batch_size)
                losses_host = losses if losses_host is None else torch.cat((losses_host, losses), dim=0)
                losses_mlm_host = losses_mlm if losses_mlm_host is None else torch.cat((losses_mlm_host, losses_mlm), dim=0)
                losses_clm_host = losses_clm if losses_clm_host is None else torch.cat((losses_clm_host, losses_clm), dim=0)
            ...
            if self.args.eval_accumulation_steps is not None and (step + 1) % self.args.eval_accumulation_steps == 0:
                ...
                eval_losses_mlm_gatherer.add_arrays(self._gather_and_numpify(losses_mlm_host, "eval_losses_mlm"))
                eval_losses_clm_gatherer.add_arrays(self._gather_and_numpify(losses_clm_host, "eval_losses_clm"))
                if not prediction_loss_only:
                    preds_gatherer.add_arrays(self._gather_and_numpify(preds_host, "eval_preds"))
                    labels_gatherer.add_arrays(self._gather_and_numpify(labels_host, "eval_label_ids"))

                # Set back to None to begin a new accumulation
                losses_host, losses_mlm_host, losses_clm_host, preds_host, labels_host = None, None, None, None, None
        ...
        eval_loss_mlm = eval_losses_mlm_gatherer.finalize()
        eval_loss_clm = eval_losses_clm_gatherer.finalize()
        ...
        if eval_loss is not None:
            ...
            metrics["eval_loss_mlm"] = eval_loss_mlm.mean().item()
            metrics["eval_loss_clm"] = eval_loss_clm.mean().item()
    ...
    def prediction_step(
        ...
        with torch.no_grad():
            ...
            if has_labels:
                if isinstance(outputs, dict):
                    ...
                    loss_mlm = outputs["loss_mlm"].mean().detach()
                    loss_clm = outputs["loss_clm"].mean().detach()
        ...
        if prediction_loss_only:
            return (loss, loss_mlm, loss_clm, None, None)
        ...
        return (loss, loss_mlm, loss_clm, logits, labels)
